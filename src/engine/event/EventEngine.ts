...
const EVENT_POOL: Omit<GameEvent, 'id'>[] = [
  ...
  { title: '美元体系崩溃', description: '多国宣布去美元化，信任危机爆发', targetAsset: 'USD', priceImpact: -0.08, sentimentImpact: -25, tier: 4 },
...
export class EventEngine {
  ...
  tick(): GameEvent | null {
    ...
    eventBus.emit('event:triggered', {
      id: event.id,
      tier: event.tier,
      direction: event.priceImpact > 0.001 ? 'up' : event.priceImpact < -0.001 ? 'down' : 'neutral',
      title: event.title,
    })
